...
//            id: 'background',
//            type: 'background',
//            paint: { 'background-color': '#0c1a36' } /* ������ɫ */
//        }
//    ]
//};
...
map.on('load', function () {
    //ȡnodes
    var apiUrl = baseApi + `/nodes/getNodesLocation`;
    var opt = { "currentPage": 1, "itemsPerPage": 100 };
    ...
    //             var nodes = data.records;// $.parseJSON(idata);
    //             var htmlList = "";
    //             for (var Idx = nodes.length - 1; Idx >= 0; Idx--) {
    //                 var iNode = nodes[Idx];
    //                 if (iNode) {
    //                     var d1 = new Date(iNode.lastOnlineTime); //ת����Data();
    //                     var d2 = new Date();
    ...
    //                     });
    //                     var htmlString = '<div class="infobox-title">Node Machine' + iNode.nodeId + '</div>'
    //                         + '<div class="infobox-info" >'
    //                         + '<div><span>Address:</span><br>' + iNode.ownerAddress + '</div>'
    //                         + '<div><span>Location:</span><br>' + iNode.local_ip + '</div>'
    //                         + '<div><span>Total STCI Rewarded:</span><br>' + iNode.total_reward + '</div>'
    //                         + '<div><span>STCI Rewarded in Last 30 Days:</span><br>' + iNode.reward_30days + '</div>'
    //                         + '<div><span>STCI Rewarded Increases:</span><br>' + iNode.increase_ratio + '</div></div>';
    ...
    //ȡsigners
    var apiUrl = baseApi + `/signers/getCommonSigners?currentPage=${1}&itemsPerPage=100`;
    $.ajax(
        {
            url: apiUrl,
            // async: false,
                ...
                for (var Idx = signers.length - 1; Idx >= 0; Idx--) {
                    ...
                    if (iSigner) {
                        ...
                        var htmlString = '<div class="infobox-title">Signer</div>'
                            ...
                            + '<div><span>Total BlockSigned:</span><br>' + iSigner.totalBlocks + '</div>'
                            + '<div><span>Latest Block Signed:</span><br>' + iSigner.maxBlock + ' </div>'
...
//    document.getElementById('info').innerHTML = JSON.stringify(e.point) + '|' + JSON.stringify(e.lngLat);  /* JSON.stringify() �������Խ������ JavaScript ֵ���л��� JSON �ַ��� */
//});
